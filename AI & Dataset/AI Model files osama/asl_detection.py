...
no_sequences = 167  # Thirty videos worth of data
sequence_length = 15  # Videos are going to be 10 frames in length
...
colors = [(0, 158, 255), (255, 54, 0), (255, 224, 0), (85, 255, 0), (0, 23, 255), (251, 0, 255), (255, 0, 0), (178, 0, 255)]
    ...
    while cap.isOpened():
        ...
        if len(sequence) == 15:
            ...
            if not all_zeros:
                ...
                array_1d = getPrediction(sequence)

                # res = model.predict(np.expand_dims(sequence, axis=0))[0]
                confidence = array_1d[np.argmax(array_1d)]
                sign = actions[np.argmax(array_1d)]
                print('The pridection is : ', sign, 'with confidence: ', confidence)

                #3. Viz logic
                if array_1d[np.argmax(array_1d)] >= threshold: 
                    if lastSign == sign:
                        print('repeated sign')
                    else:
                        lastSign = sign
                        sentence.append(actions[np.argmax(array_1d)])    

                if len(sentence) > 1: 
                    sentence = sentence[-1:]

                # Viz probabilities
                image = prob_viz(array_1d, actions, image, colors)
